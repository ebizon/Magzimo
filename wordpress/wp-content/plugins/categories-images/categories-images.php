...
wp_enqueue_script('thickbox');
	echo '<div class="form-field">
		...
		<br/>
		<button class="z_upload_image_button button" id = "z_upload_image_button">' . __('Upload/Add image', 'zci') . '</button>
	</div>'.z_script();
	echo '<div class="form-field">
		<label for="taxonomy_image1">' . __('Image', 'zci') . '</label>
		<input type="text" name="taxonomy_image1" id="taxonomy_image1" value="" />
		<br/>
		<button class="z_upload_image_button button" id = "z_upload_image_button1" >' . __('Upload/Add image', 'zci') . '</button>
	</div>'.z_script();
}
...
function z_edit_texonomy_field($taxonomy) {
	...
	echo '<tr class="form-field">
		...
		<td><img class="taxonomy-image" src="' . z_taxonomy_image_url( $taxonomy->term_id, TRUE ) . '"/><br/>
		<input type="text" name="taxonomy_image" id="taxonomy_image" value="'.$image_text.'" /><br />
		<button class="z_upload_image_button button" id = "z_upload_image_button">' . __('Upload/Add image', 'zci') . '</button>
		<button class="z_remove_image_button button" id = "z_remove_image_button" >' . __('Remove image', 'zci') . '</button>
		</td>
	</tr>'.z_script();
	if (z_taxonomy_image_url1( $taxonomy->term_id, TRUE ) == Z_IMAGE_PLACEHOLDER) 
		$image_text = "";
	else
		$image_text = z_taxonomy_image_url1( $taxonomy->term_id, TRUE );
	echo '<tr class="form-field">
		<th scope="row" valign="top"><label for="taxonomy_image1">' . __('Image', 'zci') . '</label></th>
		<td><img class="taxonomy-image1" src="' . z_taxonomy_image_url1( $taxonomy->term_id, TRUE ) . '"/><br/>
		<input type="text" name="taxonomy_image1" id="taxonomy_image1" value="'.$image_text.'" /><br />
		<button class="z_upload_image_button button" id = "z_upload_image_button1">' . __('Upload/Add image', 'zci') . '</button>
		<button class="z_remove_image_button button" id = "z_remove_image_button1" >' . __('Remove image', 'zci') . '</button>
		</td>
	</tr>'.z_script();
}
	...
	return '<script type="text/javascript">
	    jQuery(document).ready(function() {
			...
			jQuery("#z_upload_image_button").click(function() {
				...
				bid = "z_upload_image_button";
				return false;
			});
			jQuery("#z_upload_image_button1").click(function() {
				upload_button = jQuery(this);
				tb_show("", "media-upload.php?type=image&amp;TB_iframe=true");
				bid = "z_upload_image_button1";
				return false;
			});
			...
			jQuery("#z_remove_image_button1").click(function() {
				jQuery("#taxonomy_image1").val("");
				jQuery(this).parent().siblings(".title").children("img").attr("src","' . Z_IMAGE_PLACEHOLDER . '");
				jQuery(".inline-edit-col :input[name=\'taxonomy_image1\']").val("");
				return false;
			});
			window.send_to_editor = function(html) {
				...
				if (upload_button.parent().prev().children().hasClass("tax_list")) {
					...
					if(bid == "z_upload_image_button")
					jQuery("#taxonomy_image").val(imgurl);
					else
					jQuery("#taxonomy_image1").val(imgurl);
				}
				tb_remove();
			}
			jQuery(".editinline").live("click", function(){  
				...
				if (thumb != "' . Z_IMAGE_PLACEHOLDER . '") {
					...
					jQuery(".inline-edit-col :input[name=\'taxonomy_image1\']").val(thumb);
				} else {
					if(bid == "z_upload_image_button")
					jQuery(".inline-edit-col :input[name=\'taxonomy_image\']").val("");
					else
...
function z_save_taxonomy_image($term_id) {
        ...
        if(isset($_POST['taxonomy_image1']))
        update_option('z_taxonomy_image1'.$term_id, $_POST['taxonomy_image1']);
...
function z_taxonomy_image_url1($term_id = NULL, $return_placeholder = FALSE) {
	if (!$term_id) {
		if (is_category())
			$term_id = get_query_var('cat');
		elseif (is_tax()) {
	...
	if ($return_placeholder)
		return ($taxonomy_image_url1 != "") ? $taxonomy_image_url1 : Z_IMAGE_PLACEHOLDER;
	else
