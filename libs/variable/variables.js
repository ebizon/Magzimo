jQuery.extend({
    getValues: function() {
        var result = null;
        $.ajax({
            url: 'http://localhost/wordpress/?webservice_method=category',
            type: 'get',
            dataType: 'json',
            async: false,
...
result = $.getValues();
category_title = new Array();
category_id = new Array();
category_name = new Array();
category_image1 = new Array();
category_image2 = new Array();
for(i=0;i<result.length;i++)
{
	category_title[i] = result[i].title;
	category_id[i] = result[i].category_id;
	category_name[i] = result[i].category_name;
	category_image1[i] = result[i].first_image;
	category_image2[i] = result[i].second_image;
}
str ='';
for(i=0;i<3;i++)
{
	//alert(category_name[i]);
	//str = 'demo: { html: "hello" }';
	//str+= "<a href ='#' >"+category_name[i]+"</a><br>";
}
//demo = new Object();

//alert(category_name[0]);
/*
var pages_string = new Array();
for(var page_index = 0; page_index < category_title.length; page_index++){
	var pages = '';
							pages += "{"+
							"title:"+category_title[page_index]+","+					
							"pages:[h"+page_index+"]"+							
						"}";
						if(page_index != (category_title.length-1)){
							pages +=",";
						}
						pages_string[page_index] = pages;
						}
						
var pages_string1 = '';
for(var page_index = 0; page_index < category_title.length; page_index++){
							pages_string1 += page_index+":{"+category_title[page_index]+"}";
						
						
						}*/
//alert(pages_string);
//tttt = tttt.replace(/#articletitle1#/g,results[0].title)
re = '';rew = {};
function bcall()
{	
	//alert(re.demo.html);
	...
	//console.log("hi"+category_name[i]);
	var cate = category_name[i];
	rew = { cate: {'html' : 'heeeee'}};
	console.log("helo"+rew);
	$.extend(re, rew);
	}
	
	//alert(rew.cover.template);
	//alert(rew.windows1.html);
	return re;
}
function b1call()
{
	...
	for(i=0;i<category_title.length;i++){
		...
		alert(total);
		var cate = category_name[i];
			obj1 = {"title":"gotopage","subtitle":"undefined","pages":[cate]};
		//alert(category_name[i]);
		
		article1.defaultversion.articles.rollingstart[total] = obj1;
	...
	//article1.mini.articles.rollingstart.hasOwnproperty(total)
	
	
	
	alert(article1.mini.articles.rollingstart[5].pages);
  
  	return article1;
}
//alert(bcall().cover.template);
